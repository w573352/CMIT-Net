...
def get_datasets(dataset_folder, mode):
    """
        mode = train,      dataset_folder =./Data/brats2021/train
        mode = train_val  dataset_folder=./Data/brats2021/val
        mode = test       dataset_folder=./Data/brats2021/test
        dataset_folder是数据集存放的文件夹
        """
    dataset_folder = get_brats_folder(dataset_folder, mode)
    assert os.path.exists(dataset_folder), "Dataset Folder Does Not Exist1"
    """
        listdir(dataset_folder) 返回排过序的病例文件名称
    """
