...
def save_model(model,
    ...
    filename=os.path.join(args.best_folder, filename)
    torch.save(save_dict, filename)
    print('Saving checkpoint', filename)

def save_checkpoint(args, state, name="checkpoint"):
    torch.save(state, f"{args.checkpoint_folder}/{name}.pth.tar")

def save_seg_csv(args, mode, csv,filename):
    try:
        ...
        # 本来的val_metrics.to_csv(f'{str(args.csv_folder)}/metrics.csv', index=False, columns=columns)
        if args.tta == True:
            filename = filename + '_tta'
        # args.csv_folder: ./run/csv/NestedFormer
        val_metrics.to_csv(f'{str(args.csv_folder)}/{str(filename)}.csv', index=False, columns=columns)
        file_path = f'{str(args.csv_folder)}/{str(filename)}.csv'
        print("文件保存到{}".format(file_path))
...
def save_test_label(args, patient_id, predict):
    ...
    data_path = get_brats_folder(args.dataset_folder,'test')
    ...
    ## args.label_folder = ./run/label/NestedFormer
